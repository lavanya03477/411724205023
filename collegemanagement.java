...
class Principal extends Person {
    ...
    static int studid[]=new int[100]; 
    static String studname[]=new String[100]; 
    ...
    void addVP(int id,String name){
        ...
        if(isUniqueID(id)){ vpid[0]=id; vpname[0]=name; vpcount=1; System.out.println("VP added: "+name); }
        else System.out.println("Duplicate ID not allowed!");
    }
    void addHOD(int id,String name,String dept){
        for(int i=0;i<hodcount;i++) if(hoddept[i].equalsIgnoreCase(dept)){ System.out.println("HOD already exists for "+dept); return; }
        if(isUniqueID(id)){ hodid[hodcount]=id; hodname[hodcount]=name; hoddept[hodcount]=dept; hodcount++; System.out.println("HOD added: "+name+" ("+dept+")"); }
        else System.out.println("Duplicate ID not allowed!");
    }
    void addFaculty(int id,String name,String dept){
        if(isUniqueID(id)){ faculid[faculcount]=id; faculname[faculcount]=name; faculdept[faculcount]=dept; faculcount++; System.out.println("Faculty added: "+name+" ("+dept+")"); }
        else System.out.println("Duplicate ID not allowed!");
    }
    void addStudent(int id,String name,String dept){
        if(isUniqueID(id)){ studid[studcount]=id; studname[studcount]=name; studdept[studcount]=dept; studcount++; System.out.println("Student added: "+name+" ("+dept+")"); }
        else System.out.println("Duplicate ID not allowed!");
    }

    // Update methods
    void updateVP(int id,String newName){ if(vpcount==1 && vpid[0]==id){ vpname[0]=newName; System.out.println("VP updated."); } else System.out.println("VP not found."); }
    void updateHOD(int id,String newName){ for(int i=0;i<hodcount;i++) if(hodid[i]==id){ hodname[i]=newName; System.out.println("HOD updated."); return; } System.out.println("HOD not found."); }
    void updateFaculty(int id,String newName){ for(int i=0;i<faculcount;i++) if(faculid[i]==id){ faculname[i]=newName; System.out.println("Faculty updated."); return; } System.out.println("Faculty not found."); }
    void updateStudent(int id,String newName){ for(int i=0;i<studcount;i++) if(studid[i]==id){ studname[i]=newName; System.out.println("Student updated."); return; } System.out.println("Student not found."); }

    // Delete methods
    void deleteVP(int id){ if(vpcount==1 && vpid[0]==id){ vpcount=0; System.out.println("VP deleted."); } else System.out.println("VP not found."); }
    void deleteHOD(int id){ for(int i=0;i<hodcount;i++) if(hodid[i]==id){ for(int j=i;j<hodcount-1;j++){ hodid[j]=hodid[j+1]; hodname[j]=hodname[j+1]; hoddept[j]=hoddept[j+1]; } hodcount--; System.out.println("HOD deleted."); return; } System.out.println("HOD not found."); }
    void deleteFaculty(int id){ for(int i=0;i<faculcount;i++) if(faculid[i]==id){ for(int j=i;j<faculcount-1;j++){ faculid[j]=faculid[j+1]; faculname[j]=faculname[j+1]; faculdept[j]=faculdept[j+1]; } faculcount--; System.out.println("Faculty deleted."); return; } System.out.println("Faculty not found."); }
    void deleteStudent(int id){ for(int i=0;i<studcount;i++) if(studid[i]==id){ for(int j=i;j<studcount-1;j++){ studid[j]=studid[j+1]; studname[j]=studname[j+1]; studdept[j]=studdept[j+1]; } studcount--; System.out.println("Student deleted."); return; } System.out.println("Student not found."); }
}
...
class VicePrincipal extends Person {
    ...
    void viewdetails(){
        ...
        for(int i=0;i<Principal.hodcount;i++) System.out.println("HOD: "+Principal.hodname[i]+" ("+Principal.hoddept[i]+") ID:"+Principal.hodid[i]);
        for(int i=0;i<Principal.faculcount;i++) System.out.println("Faculty: "+Principal.faculname[i]+" ("+Principal.faculdept[i]+") ID:"+Principal.faculid[i]);
        for(int i=0;i<Principal.studcount;i++) System.out.println("Student: "+Principal.studname[i]+" ("+Principal.studdept[i]+") ID:"+Principal.studid[i]);
    }

    @Override
    void managetasks(){ System.out.println("Vice Principal can add, update, delete HOD, Faculty, Students."); }

    // Add methods
    void addHOD(int id,String name,String dept){ Principal.hodid[Principal.hodcount]=id; Principal.hodname[Principal.hodcount]=name; Principal.hoddept[Principal.hodcount]=dept; Principal.hodcount++; System.out.println("HOD added: "+name+" ("+dept+")"); }
    void addFaculty(int id,String name,String dept){ Principal.faculid[Principal.faculcount]=id; Principal.faculname[Principal.faculcount]=name; Principal.faculdept[Principal.faculcount]=dept; Principal.faculcount++; System.out.println("Faculty added: "+name+" ("+dept+")"); }
    void addStudent(int id,String name,String dept){ Principal.studid[Principal.studcount]=id; Principal.studname[Principal.studcount]=name; Principal.studdept[Principal.studcount]=dept; Principal.studcount++; System.out.println("Student added: "+name+" ("+dept+")"); }

    // Update methods
    void updateHOD(int id,String newName){
        for(int i=0;i<Principal.hodcount;i++){
            if(Principal.hodid[i]==id){ Principal.hodname[i]=newName; System.out.println("HOD updated."); return; }
        }
        System.out.println("HOD not found.");
    }
    void updateFaculty(int id,String newName){
        for(int i=0;i<Principal.faculcount;i++){
            if(Principal.faculid[i]==id){ Principal.faculname[i]=newName; System.out.println("Faculty updated."); return; }
        }
        System.out.println("Faculty not found.");
    }
    void updateStudent(int id,String newName){
        for(int i=0;i<Principal.studcount;i++){
            if(Principal.studid[i]==id){ Principal.studname[i]=newName; System.out.println("Student updated."); return; }
        }
        ...
        for(int i=0;i<Principal.hodcount;i++){
            if(Principal.hodid[i]==id){
                for(int j=i;j<Principal.hodcount-1;j++){
                    ...
                    Principal.hoddept[j]=Principal.hoddept[j+1];
                }
                Principal.hodcount--;
        ...
        for(int i=0;i<Principal.faculcount;i++){
            if(Principal.faculid[i]==id){
                for(int j=i;j<Principal.faculcount-1;j++){
                    ...
                    Principal.faculdept[j]=Principal.faculdept[j+1];
                }
        ...
        for(int i=0;i<Principal.studcount;i++){
            if(Principal.studid[i]==id){
                for(int j=i;j<Principal.studcount-1;j++){
                    ...
                    Principal.studdept[j]=Principal.studdept[j+1];
                }
                Principal.studcount--;
